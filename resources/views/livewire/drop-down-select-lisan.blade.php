            ...
            <select class="form-control" wire:model="selectedCode" 
                ...
                @if ($codes)
                    @foreach ($codes as $code)
                        <option value="{{ $code->id }}">{{ $code->code_lisan_name }}</option>
                    @endforeach
                @endif
            </select>
        </div>
                  ...
                  @foreach ($criterias as $criteria)
                    <div class="form-group m-0">
                        <div class="form-check">
                            ...
                            @if (DB::table('pertanyaan_lisan')->where('code_lisan_id', $code_id)->where('unit_id', $aunit->id)->where('no_soal','LIKE', '%'.$criteria->element->no_element.'.'.$criteria->no_criteria.'%')->count() > 0)
                                disabled
                            @endif
                            >
